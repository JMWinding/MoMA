...
const unsigned int ChipInterval = 200;
...
// **** Pump Parameters ****
const int nTx = 6;
const int motors[nTx] = {2, 3, 4, 5, 6, 7};
const int motorOffsets[nTx] = {20, 20+14* (1+prelen)*1, 20+14* (1+prelen)*2, 20+14* (1+prelen)*3, 20+14* (1+prelen)*4, 20+14* (1+prelen)*5};
const float motorCon[nTx] = {10, 10, 10, 10, 10, 10};
...
// tx4
void tx4_transmit_over (void) { tx_transmit_over (4); }
Ticker timerTx4Off (tx4_transmit_over, PulseOnTime, 1, MILLIS);
void tx4_transmit (void) { txi_transmit (4); timerTx4Off.start (); }
Ticker timerTx4On (tx4_transmit, ChipInterval, 0, MILLIS);
int tx4OnInitCounter = 0;
void tx4OnInit (void) { 
  if (--tx4OnInitCounter == 0) {
    pchip[4] = 0; pind[4] = 0; xbit[4] = 1; xind[4] = 0; timerTx4On.start ();
  }
}
Ticker timerTx4OnInit (tx4OnInit, ChipInterval, 0, MILLIS);

// tx5
void tx5_transmit_over (void) { tx_transmit_over (5); }
Ticker timerTx5Off (tx5_transmit_over, PulseOnTime, 1, MILLIS);
void tx5_transmit (void) { txi_transmit (5); timerTx5Off.start (); }
Ticker timerTx5On (tx5_transmit, ChipInterval, 0, MILLIS);
int tx5OnInitCounter = 0;
void tx5OnInit (void) { 
  if (--tx5OnInitCounter == 0) {
    pchip[5] = 0; pind[5] = 0; xbit[5] = 1; xind[5] = 0; timerTx5On.start ();
  }
}
Ticker timerTx5OnInit (tx5OnInit, ChipInterval, 0, MILLIS);
...
void loop () {
  ...
  for (int fileIndex = fileIndexStart; fileIndex <= fileIndexEnd; fileIndex++)
  {
    ...
    tx4OnInitCounter = motorOffsets[4]+motorOffsets2[4];
    tx5OnInitCounter = motorOffsets[5]+motorOffsets2[5];
    ...
    timerTx4OnInit.start ();
    timerTx5OnInit.start ();
    while (1)
    {
      ...
      timerTx3Off.update ();
      timerTx4OnInit.update ();
      if (timerTx4On.state () == status_t::RUNNING) { timerTx4OnInit.stop (); }
      timerTx4On.update ();
      if (timerTx4On.counter () >= totalChips) { timerTx4On.stop (); }
      timerTx4Off.update ();
      timerTx5OnInit.update ();
      if (timerTx5On.state () == status_t::RUNNING) { timerTx5OnInit.stop (); }
      timerTx5On.update ();
      if (timerTx5On.counter () >= totalChips) { timerTx5On.stop (); }
      timerTx5Off.update ();
      ...
      if (timerTx4OnInit.state () != status_t::STOPPED) { continue; }
      if (timerTx4On.state () != status_t::STOPPED) { continue; }
      if (timerTx4Off.state () != status_t::STOPPED) { continue; }
      if (timerTx5OnInit.state () != status_t::STOPPED) { continue; }
      if (timerTx5On.state () != status_t::STOPPED) { continue; }
      if (timerTx5Off.state () != status_t::STOPPED) { continue; }
